  ...
  return function eventDrops (config) {
    ...
    function eventDropGraph (selection) {
      selection.each(function (data) {
        ...
        //添加按钮zwxg
        var iszoom=1;
        var zoomButton = d3.select("#eventstory1")
          .append('input')
          .attr("type","button")
          .attr("value","缩放")
          .on("click",function(){
              if(iszoom==1) {d3.select("#zoomrect").attr("display","none");iszoom=0;this.value="查询";}
              else {d3.select("#zoomrect").attr("display","block");iszoom=1;this.value="缩放";}
          });
    ...
    var eventLine = function eventLine(selection) {
      selection.each(function (data) {
        ...
        circle.enter()
          ...
          //.on("mouseover",function(d,i){d3.select("#zoomrect").attr("display","none");tipshow(d,"eventstory1");console.log(d);})//zwxg
          //.on("mouseout",function(){d3.select("#zoomrect").attr("display","block");tiphide();})//zwxg
          //.on("click",function(d){d3.select("#zoomrect").attr('display', 'none');d3.select("#zoomrect").attr('display', 'block');})
        ;
