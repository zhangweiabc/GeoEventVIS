//ComicNanrrative
function DrawEventstory2(divID,width,height)
{
	// Link dimensions
	var link_width = 1.8;
	var link_gap = 2;
	...
	var colornarrative = d3.scale.category10();
	var raw_chart_width = width;
	var raw_chart_height = height;
	...
	function draw_links(links, svg, safe_name) {
	    var link = svg.append("g").selectAll(".link")
					...
					.style("stroke", function(d) { return d3.rgb(colornarrative(d.group)).darker(0.5).toString(); })
	...
	function draw_chart(name, safe_name, folder, tie_breaker, center_sort, collapse) {
		...
		d3.xml(folder + "/characters.xml", function(x) {
		    ...
		    */
		    var svg = d3.select(divID).append("text")
		        .attr("x", 0)
				.attr("y", 0)
				.attr("dy", ".35em")
				.attr("text-anchor", "end")
	...
	dir = "data/";
	draw_chart("时间轴, \"一大\"", "Myeventstory2", dir + "Comic_narrative", true, false, false);
}