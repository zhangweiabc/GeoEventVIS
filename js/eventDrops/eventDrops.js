  ...
  return function eventDrops (config) {
    ...
    function eventDropGraph (selection) {
      selection.each(function (data) {
        ...
        //添加按钮zwxg
        d3.select(this).select('input').remove();
        var iszoom=1;
        var zoomButton = d3.select(this)
          .append('input')
          .attr("type","button")
          .attr("value","缩放")
          .on("click",function(){
              if(iszoom==1) {
                d3.select("#zoomrect").attr("display","none");
                iszoom=0;
                this.value="查询";
                //config.updateView(new Date(2010, 3),new Date(2010, 5));
              }
              else {
                d3.select("#zoomrect").attr("display","block");
                iszoom=1;
                this.value="缩放";
              }
          });//zwxg
        ...
        var curx, cury;
        var zoomRect = svg.append('rect').attr("id","zoomrect")
                          .call(zoom)
                          .classed('zoom', true)
        ...
        zoom.size([config.width, height]);
          ...
          lines.exit().remove();
        //zwxg
        /*config.updateView = function (datestart,dateend)
        {
            console.log("ddda");
            config.start = datestart;
            config.end = dateend;
      ...
      selection.each(function (data) {
        ...
        var circle = d3.select(this).selectAll('circle')
          .data(function(d) {
            ...
            var da = filterData(d.events, config.xScale);//zwxg
            return da;
          });
        //zwxg
        ...
        circle.enter()
              .append('circle')
              .attr("class","circlevent")
              .attr('cx', function(d) {
                return config.xScale(d.date);})
              .style('fill', config.eventColor)
              .attr('cy', -5)
              .attr('fill-opacity',0.5)
              .attr('r', function(d){return rScale(d.nkill)||0;})//zwxg
              .on("mouseover",function(d){tipshow("死亡人数："+d["nkill"],"eventstory1");})//zwxg
              .on("mouseout",function(){tiphide();})//zwxg
              .on("click",function(d){})//zwxg
            ;
